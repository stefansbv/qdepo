...
# ABSTRACT: The QDepo application framework and runtime
use strict;
use warnings;
...
sub create_classicmodels {
    ...
    $dbh->do($sql_text) or die $dbh->errstr;
...
sub get_sql_filename {
    return catfile( dist_dir('QDepo'), 'cm', 'sql', 'classicmodels-si.sql' );
}

sub get_data_dir {
    return catdir( dist_dir('QDepo') ,'cm', 'data');
}
...
This script is part of the QDepo application.
