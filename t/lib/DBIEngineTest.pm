...
sub run {
    ...
    my $class           = $p{class};
    #my $mock_transfer   = Test::MockModule->new('QDepo');
    ...
    subtest 'live database' => sub {
        ...
        isa_ok $target, 'QDepo::Target', 'target';
        ...
        throws_ok { $engine->dbh->do('INSERT blah INTO __bar_____') }
            'QDepo::X',
            'Database error should be converted to QDepo exception';
        ...
        ######################################################################
        if ($class eq 'QDepo::Engine::pg') {
            # Test someting specific for Pg
        }


        ######################################################################
        # Test table_info_short
        ...
        ok my $info = $engine->table_info_short($table_info), 'get info for table';
        ...
        throws_ok { $engine->table_info_short }
...
END_SQL
        ...
        if ($class eq 'QDepo::Engine::sqlite') {
